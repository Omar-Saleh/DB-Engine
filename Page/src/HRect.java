...
class HRect implements Serializable{
	...
	for (int i=0; i<t.coord.length; ++i) {
           ...
           if (((Integer) t.coord[i]).compareTo((Integer) min.coord[i])<= 0) {
               p.coord[i] = min.coord[i];
           }
           else if (((Integer) t.coord[i]).compareTo((Integer) min.coord[i])>= 0) {
	...
	for (int i=0; i<min.coord.length; ++i) {
	    newmin.coord[i] = (((Integer) min.coord[i]).compareTo((Integer) r.min.coord[i]))> 0 ? min.coord[i] : r.min.coord[i];
        newmax.coord[i] = (((Integer) max.coord[i]).compareTo((Integer) r.max.coord[i]))> 0 ? max.coord[i] : r.max.coord[i];

	    if (((Integer) newmin.coord[i]).compareTo((Integer) newmax.coord[i]) >= 0) return null;
	}
	...
	for (int i=0; i<min.coord.length; ++i) {
	   // a *= (((Integer) max.coord[i]).diff(min.coord[i]));
		a *= diff(max.coord[i], min.coord[i]);
	}
