...
class KDNode implements Serializable{
    ...
    protected static KDNode ins(HPoint key, Point val, KDNode t, int lev, int K)  {
    ...
    protected static void rsearch(HPoint lowk, HPoint uppk, KDNode t, int lev, int K, Vector<KDNode> v) {
        ...
        if (t == null)
            return;
        if (((Double) lowk.coord[lev]).compareTo((Double) t.k.coord[lev])> 0) {
            rsearch(lowk, uppk, t.left, (lev + 1) % K, K, v);
        }
        int j;
        for (j = 0; j < K && ((Double) lowk.coord[lev]).compareTo((Double) t.k.coord[lev])<= 0
                && ((Double) uppk.coord[lev]).compareTo((Double) t.k.coord[lev])>= 0; j++)
            ;
        if (j == K)
            v.add(t);
        if (((Double) uppk.coord[lev]).compareTo((Double) t.k.coord[lev])> 0) {
    ...
    protected static void nnbr(KDNode kd, HPoint target, HRect hr, double max_dist_sqd, int lev, int K,
        ...
        boolean target_in_left = ((Double) target.coord[lev]).compareTo((Double) pivot.coord[lev]) < 0;;
    ...
    private KDNode(HPoint key, Point val) {
